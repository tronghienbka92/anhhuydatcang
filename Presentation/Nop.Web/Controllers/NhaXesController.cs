...
namespace Nop.Web.Controllers
{
    public class NhaXesController : BaseNhaXeController
    {
        ...
        protected virtual void XeInfoToXeInfoModel(XeVanChuyen nvfrom, XeInfoModel nvto)
        {
            ...
            if (nvfrom.phuxe != null)
                nvto.PhuXeText = nvfrom.phuxe.HoVaTen;
            nvto.BienSo = nvfrom.BienSo;
            nvto.DienThoai = nvfrom.DienThoai;
        }
        [NonAction]
        protected virtual void XeInfoModelPrepare(XeInfoModel model)
        {
            ...
            model.PhuXes = _nhanvienService.GetAll(_workContext.NhaXeId).Where(c => c.KieuNhanVienID == (int)ENKieuNhanVien.PhuXe).Select(c =>
            {
                var item = new SelectListItem();
                item.Text = c.HoVaTen;
                item.Value = c.Id.ToString();
        ...
        #endregion
        #region Nguon ve xe
        public ActionResult CbbListDiemDon(string SearchDiemDon)
        {
            var diemdons = _hanhtrinhService.GetAllDiemDonByNhaXeId(_workContext.NhaXeId).Where(c => c.LoaiDiemDon == ENLoaiDiemDon.DiemBatKhachDocDuong).ToList();
            return Json(diemdons, JsonRequestBehavior.AllowGet);
        }
        [NonAction]
        void NguonVeXeBatDocDuongToMode(NguonVeXe e, NguonVeXeModel m)
        {
            m.Id = e.Id;
        ...
        public ActionResult NguonVeXeBatDocDuongList()
        {
            if (this.CheckNoAccessIntoNhaXe(_workContext, _permissionService, StandardPermissionProvider.CVHanhTrinh))
                return AccessDeniedView();
        ...
        [HttpPost]
        public ActionResult NguonVeXeBatDocDuongList(DataSourceRequest command, NguonVeListModel model)
        {
            if (this.CheckNoAccessIntoNhaXe(_workContext, _permissionService, StandardPermissionProvider.CVHanhTrinh))
                return AccessDeniedView();


            var items = _hanhtrinhService.GetAllNguonVeXe(_workContext.NhaXeId, 0, model.HanhTrinhId).Where(c => c.DiemDonGoc.LoaiDiemDon == ENLoaiDiemDon.DiemBatKhachDocDuong).ToList();
            var nguonves = items
                .Select(c =>
                {
                    var item = new NguonVeXeModel();
                    NguonVeXeBatDocDuongToMode(c, item);
        ...
        public ActionResult VeXeBatDocDuongTao()
        {
            if (this.CheckNoAccessIntoNhaXe(_workContext, _permissionService, StandardPermissionProvider.CVHanhTrinh))
                return AccessDeniedView();
            ...
            model.LichTrinhs = _hanhtrinhService.GetAllLichTrinhByHanhTrinhId(0, _workContext.NhaXeId).Select(c =>
            {
                var item = new SelectListItem();
                item.Text = string.Format("{0} ({1})", c.HanhTrinhInfo.MoTa, c.MaLichTrinh);
                item.Value = c.Id.ToString();
                return item;
            }).ToList();
            model.LichTrinhs.Insert(0, new SelectListItem { Text = "Chọn lịch trình", Value = "0" });
            var diemdondis = _hanhtrinhService.GetAllDiemDonByNhaXeId(_workContext.NhaXeId).Where(c=>c.LoaiDiemDon==ENLoaiDiemDon.DiemBatKhachDocDuong).ToList();
            model.DiemDonDis = diemdondis.Select(c =>
                {
                    var item = new SelectListItem();
                    item.Text = c.TenDiemDon;
                    item.Value = c.Id.ToString();
            ...
            model.DiemDonDis.Insert(0, new SelectListItem { Text = "Chọn điểm đón", Value = "0" });

            model.DiemDonDens = _hanhtrinhService.GetAllDiemDonByNhaXeId(_workContext.NhaXeId)
                .Where(c => c.LoaiDiemDon == ENLoaiDiemDon.DiemBatKhachDocDuong).ToList().Select(c =>
                {
                    var item = new SelectListItem();
                    item.Text = c.TenDiemDon;
                    item.Value = c.Id.ToString();
            ...
            model.DiemDonDens.Insert(0, new SelectListItem { Text = "Chọn điểm đón", Value = "0" });
            return View(model);
        }
        [HttpPost, ParameterBasedOnFormName("save-continue", "continueEditing")]
        public ActionResult VeXeBatDocDuongThemSua(NguonVeListModel model)
        {
            if (this.CheckNoAccessIntoNhaXe(_workContext, _permissionService, StandardPermissionProvider.CVHanhTrinh))
                return AccessDeniedView();          
            if(model.Id>0)
            {
                ...
                item.DiemDonId = model.DiemDonDiId;
                item.DiemDenId = model.DiemDonDenId;
                item.DiemDonGocId = model.DiemDonDiId;
                item.DiemDenGocId = model.DiemDonDenId;
                item.LichTrinhId = model.LichTrinhId;
                item.GiaVeHienTai = model.GiaVeHienTai;
                _hanhtrinhService.UpdateNguonVeXe(item);
                return Content("OK");
            }
            else
            {
                var nguonvexes = _hanhtrinhService.GetAllNguonVeXe(_workContext.NhaXeId, 0, 0, model.DiemDonDiId, model.DiemDonDenId, ENKhungGio.All);
                if (nguonvexes.Count > 0)
                    return Content("Not OK");
                var item = new NguonVeXe();
                item.NhaXeId = _workContext.NhaXeId;
                item.LichTrinhId = model.LichTrinhId;
                item.DiemDonId = model.DiemDonDiId;
                item.DiemDenId = model.DiemDonDenId;
                item.TenNhaXe = _nhaxeService.GetNhaXeById(_workContext.NhaXeId).TenNhaXe;
                item.GiaVeHienTai = model.GiaVeHienTai;
                item.DiemDonGocId = model.DiemDonDiId;
                item.DiemDenGocId = model.DiemDonDenId;
                item.TenDiemDon = _hanhtrinhService.GetDiemDonById(model.DiemDonDiId).TenDiemDon;
                item.TenDiemDen = _hanhtrinhService.GetDiemDonById(model.DiemDonDenId).TenDiemDon;
                item.ThoiGianDi = DateTime.Now;
                item.ThoiGianDen = DateTime.Now;
                item.ProductId = _productService.GetAllProduct().First().Id;
                item.LoaiXeId = _xeinfoService.GetAllByNhaXeId(_workContext.NhaXeId).First().Id;

                _hanhtrinhService.InsertNguonVeXe(item);
                return Content("OK");
            }
            
        }
        public ActionResult VeXeBatDocDuongSua(int id)
        {
            if (this.CheckNoAccessIntoNhaXe(_workContext, _permissionService, StandardPermissionProvider.CVHanhTrinh))
                return AccessDeniedView();

            var model = new NguonVeListModel();
            var nguonve = _hanhtrinhService.GetNguonVeXeById(id);
            if (nguonve == null)
                return RedirectToAction("NguonVeXeBatDocDuongList");
            model.Id = nguonve.Id;
            model.DiemDonDiId = nguonve.DiemDonId;
            model.DiemDonDenId = nguonve.DiemDenId;
            model.LichTrinhId = nguonve.LichTrinhId;
            model.GiaVeHienTai = nguonve.GiaVeHienTai;
            model.LichTrinhs = _hanhtrinhService.GetAllLichTrinhByHanhTrinhId(0, _workContext.NhaXeId).Select(c =>
            {
                var item = new SelectListItem();
                item.Text = string.Format("{0} ({1})", c.HanhTrinhInfo.MoTa, c.MaLichTrinh);
                item.Value = c.Id.ToString();
                item.Selected = c.Id == nguonve.LichTrinhId;
                return item;
            }).ToList();
            model.DiemDonDis = _hanhtrinhService.GetAllDiemDonByNhaXeId(_workContext.NhaXeId)
                .Where(c => c.LoaiDiemDon == ENLoaiDiemDon.DiemBatKhachDocDuong).ToList().Select(c =>
            {
                var item = new SelectListItem();
                item.Text = c.TenDiemDon;
                item.Value = c.Id.ToString();
                item.Selected = c.Id == nguonve.DiemDonId;
                return item;
            }).ToList();
            model.DiemDonDens = _hanhtrinhService.GetAllDiemDonByNhaXeId(_workContext.NhaXeId)
                .Where(c => c.LoaiDiemDon == ENLoaiDiemDon.DiemBatKhachDocDuong).ToList().Select(c =>
            {
                ...
                item.Text = c.TenDiemDon;
                item.Value = c.Id.ToString();
                item.Selected = c.Id == nguonve.DiemDenId;
                return item;
            }).ToList();
            return View(model);
        }
        public ActionResult NguonVeXeBatDocDuongXoa(int Id)
        {
            if (this.CheckNoAccessIntoNhaXe(_workContext, _permissionService, StandardPermissionProvider.CVHanhTrinh))
                return AccessDeniedView();
            var item = _hanhtrinhService.GetNguonVeXeById(Id);
            if (item != null)
                return RedirectToAction("NguonVeXeBatDocDuongList");
            _hanhtrinhService.DeleteNguonVeXe(item);
            return RedirectToAction("NguonVeXeBatDocDuongList");
        }
        public ActionResult _VeXeBatDocDuongChinhSua(NguonVeListModel model)
        {
            return View();
        }
        public ActionResult QLVeXeKhachBatDocDuong()
        {
            if (this.CheckNoAccessIntoNhaXe(_workContext, _permissionService, StandardPermissionProvider.CVHanhTrinh))
                return AccessDeniedView();
            var model = new NguonVeListModel();
        ...
        [HttpPost]
        public ActionResult ThemMoiPhoiVeKhachBatDocDuong(NguonVeListModel model)
        {
            if (this.CheckNoAccessIntoNhaXe(_workContext, _permissionService, StandardPermissionProvider.CVHanhTrinh))
                return AccessDeniedView();
            var nguonvexes = _hanhtrinhService.GetAllNguonVeXe(_workContext.NhaXeId, 0, 0, model.DiemDonDiId, model.DiemDonDenId, ENKhungGio.All);

            //var chuyendi = _nhaxeService.GetChuyenDiById(model.ChuyenDiId);
            if (nguonvexes.Count()==0)
                return Content("Not OK");
            var nguonvexe = nguonvexes.First();
            var item = new PhoiVe();
            item.NguonVeXeId = nguonvexe.Id;
            item.NgayTao = DateTime.Now;
            item.TrangThaiId = (int)ENTrangThaiPhoiVe.DaThanhToan;
            item.CustomerId = _workContext.CurrentNhanVien.Id;
            item.SoDoGheXeQuyTacId = 4907;
            item.NgayDi = DateTime.Now;
            item.NgayUpd = DateTime.Now;
            item.NguoiDatVeId = _workContext.CurrentNhanVien.Id;
            item.GiaVeHienTai = nguonvexe.GiaVeHienTai * model.SoLuongVe;
            //item.ChangId = 735;
            item.ChuyenDiId = model.ChuyenDiId;
            _phoiveService.InsertPhoiVe(item);
            return Content("OK");
        }
        public ActionResult GetGiaVeBatDocDuong(int DiemDonId,int DiemDenId)
        {
            var nguonvexes = _hanhtrinhService.GetAllNguonVeXe(_workContext.NhaXeId, 0, 0, DiemDonId, DiemDenId, ENKhungGio.All);
            var GiaVe="";
            if (nguonvexes.Count() > 0)
                GiaVe = nguonvexes.FirstOrDefault().GiaVeHienTai.ToString();
            return Json(GiaVe, JsonRequestBehavior.AllowGet);
        }
        public ActionResult _QLVeXeKhachBatDocDuongChinhSua()
        {
            return View();
        }
        public ActionResult CBBChuyenDi(string chuyendi, int DiemDonDiId, int DiemDonDenId, string NgayTao)
        {
            DateTime _ngaytao = DateTime.Parse(NgayTao).Date;
            //var ddlchuyendis = _nhaxeService.GetAllChuyenDiTrongNgay(_workContext.NhaXeId, _ngaytao, 0, ENKhungGio.All, "", false, 0, false);
            var ddlchuyendis = _nhaxeService.GetAllChuyenDiTrongNgay(_workContext.NhaXeId, _ngaytao, 0, ENKhungGio.All, "", false, 0, false).Select(c =>
            {
                var item = new CustomerNhaXeModel();
                item.Id = c.Id;
