...
using Nop.Web.vn.thongbaonhanh.ws;
...
namespace Nop.Web.Controllers
{
    public class HangHoaController : BaseNhaXeController
    {
        ...
        public string SendSMS(string to, string text)
        {
            ...
            string brandname = "Ahuydatcang";
            string username = "ahuydatcang";
            string pwd = "Vnpt@123";
            var result = s.SendSMSBrn(brandname, username, pwd, to, text);
            return result;
        }
        ...
        {
            var biensos = _xeinfoService.GetAllXeVanChuyenByNhaXeId(_workContext.NhaXeId).Where(c => c.TrangThaiXeId != (int)ENTrangThaiXe.Huy).Select(c =>
            {
                ...
                if (c.phuxe != null)
                    item.PhuXeText = c.phuxe.HoVaTen;
                return item;
            }).ToList();
        ...
        public ActionResult QLGuiHang()
        {
            ...
            //var test = SendSMS("841657556243", "anh huy dat cang");
            //var test1 = SendSMS("841657556243", test);
            return View(model);
        }
        ...
        #endregion
        #region phieu nhan tren xe
        public ActionResult QLPhieuBienNhanTrenXe()
        {
            if (this.CheckNoAccessIntoNhaXe(_workContext, _permissionService, StandardPermissionProvider.CVHangHoaKiGui))
            {
                if (this.CheckNoAccessIntoNhaXe(_workContext, _permissionService, StandardPermissionProvider.CVQLChuyen))
        ...
        public ActionResult _PhieuBienNhanChinhSuaTrenXe1(int? Id)
        {
            if (this.CheckNoAccessIntoNhaXe(_workContext, _permissionService, StandardPermissionProvider.CVHangHoaKiGui))
                return AccessDeniedView();
            var model = new PhieuChuyenPhatModel();
            ...
            else
            {
                ...
                model.VanPhongGuiId = _workContext.CurrentVanPhong.Id;
                model.VanPhongGuiText = _workContext.CurrentVanPhong.TenVanPhong;
                var items = _phieuchuyenphatService.GetAllBangGiaCuocHangHoa(_workContext.NhaXeId);
                var itemModel = items.Select(x =>
                {
                    var m = new BangGiaCuocHangHoaModel();
                    BangGiaCuocHangHoaToModel(x, m);
                    return m;
                }).ToList();
                model.banggiacuochanghoas = itemModel;
            }
            PhieuChuyenPhatPrepareModel(model);
            return PartialView(model);
        }
        public ActionResult _PhieuBienNhanChinhSuaTrenXe(int? Id)
        {
            if (this.CheckNoAccessIntoNhaXe(_workContext, _permissionService, StandardPermissionProvider.CVHangHoaKiGui))
                return AccessDeniedView();
            var model = new PhieuChuyenPhatModel();
        ...
        [HttpPost]
        public ActionResult PhieuBienNhanTrenXeChinhSua(PhieuChuyenPhatModel model)
        {
            if (this.CheckNoAccessIntoNhaXe(_workContext, _permissionService, StandardPermissionProvider.CVHangHoaKiGui))
                return AccessDeniedView();
            ...
            {
                //tao moi phiếu chuyển phát              
                JavaScriptSerializer j = new JavaScriptSerializer();
                var bangcuocgiahanghoas = new List<PhieuChuyenPhatModel.BangCuocGiaHangHoaModel>();
                bangcuocgiahanghoas = j.Deserialize<List<PhieuChuyenPhatModel.BangCuocGiaHangHoaModel>>(model.TenHang);
                foreach (var item in bangcuocgiahanghoas)
                {
                    //PhieuChuyenPhatModelToPhieuChuyenPhat(phieuchuyenphat, model);
                    if (item.SoLuong > 0)
                    {
                        ...
                        phieuchuyenphat.NhanVienGiaoDichId = _workContext.CurrentNhanVien.Id;
                        phieuchuyenphat.VanPhongGuiId = _workContext.CurrentVanPhong.Id;
                        phieuchuyenphat.VanPhongNhanId = model.VanPhongNhanId;
                        phieuchuyenphat.NguoiGuiId = 81575;
                        phieuchuyenphat.NguoiNhanId = 81575;
                        var bangcuocgiahanghoa = _phieuchuyenphatService.GetBangCuocGiaHangHoaById(item.Id);
                        phieuchuyenphat.TenHang = bangcuocgiahanghoa.TenHangHoa;
                        phieuchuyenphat.CuocPhi = bangcuocgiahanghoa.GiaCuoc * item.SoLuong;
                        phieuchuyenphat.CuocCapToc = 0;
                        phieuchuyenphat.CuocGiaTri = 0;
                        phieuchuyenphat.CuocTanNoi = 0;
                        phieuchuyenphat.CuocNhanTanNoi = 0;
                        phieuchuyenphat.CuocVuotTuyen = 0;
                        phieuchuyenphat.TongCuocDaThanhToan = 0;
                        phieuchuyenphat.NgayNhanHang = model.NgayNhanHang;
                        phieuchuyenphat.TrangThai = ENTrangThaiChuyenPhat.KetThuc;
                        phieuchuyenphat.LoaiPhieuId = (int)ENLoaiPhieuChuyenPhat.ThuTanNoi;
                        phieuchuyenphat.IsPhieuVanChuyenTrenXe = true;
                        _phieuchuyenphatService.InsertPhieuChuyenPhat(phieuchuyenphat);
                        _phieuchuyenphatService.InsertPhieuChuyenPhatLog("Phiếu được tạo bởi " + _workContext.CurrentNhanVien.HoVaTen, phieuchuyenphat.Id);
                        //tạo mới phiếu vận chuyển
                        var phieuvanchuyen = new PhieuVanChuyen();
                        phieuvanchuyen.NhaXeId = _workContext.NhaXeId;
                        phieuvanchuyen.TrangThai = ENTrangThaiPhieuVanChuyen.KetThuc;
                        var vanphongnhan = _nhaxeService.GetVanPhongById(model.VanPhongNhanId);
                        phieuvanchuyen.KhuVucDenId = vanphongnhan.khuvuc.Id;
                        phieuvanchuyen.LoaiPhieuVanChuyenId = (int)ENLoaiPhieuVanChuyen.TrongTuyen;
                        ...
                        phieuvanchuyen.SoLenh = string.Format("{0}-{1}", _workContext.CurrentVanPhong.Ma, _solenhnum);
                        _phieuchuyenphatService.InsertPhieuVanChuyen(phieuvanchuyen);
                        //update phieuvanchuyenid cho phieuchuyenphat
                        phieuchuyenphat.PhieuVanChuyenId = phieuvanchuyen.Id;
                        _phieuchuyenphatService.UpdatePhieuChuyenPhat(phieuchuyenphat);
                        //tạo log phiếu vận chuyển
                        var phieuvanchuyenlog = new PhieuVanChuyenLog();
                        phieuvanchuyenlog.PhieuVanChuyenId = phieuvanchuyen.Id;
                        phieuvanchuyenlog.ChuyenDiId = model.ChuyenDiId;
                        var chuyendi = _nhaxeService.GetHistoryXeXuatBenId(model.ChuyenDiId);
                        phieuvanchuyenlog.HanhTrinhId = chuyendi.HanhTrinhId;
                        phieuvanchuyenlog.TuyenId = chuyendi.HanhTrinh.TuyenVanDoanhId;
                        phieuvanchuyenlog.VanPhongGuiId = _workContext.CurrentVanPhong.Id;
                        phieuvanchuyenlog.VanPhongNhanId = model.VanPhongNhanId;
                        phieuvanchuyenlog.KhuVucId = phieuchuyenphat.VanPhongNhan.khuvuc.Id;
                        phieuvanchuyenlog.NguoiGiaoId = _workContext.CurrentNhanVien.Id;
                        phieuvanchuyenlog.NguoiNhanId = 1;
                        phieuvanchuyenlog.NgayTao = DateTime.Now;
        ...
        [HttpPost]
        public ActionResult _PhieuVanChuyenTrenXeList(ListPhieuModel model)
        {

            if (this.CheckNoAccessIntoNhaXe(_workContext, _permissionService, StandardPermissionProvider.CVHangHoaKiGui))
                return AccessDeniedView();
            var items = _phieuchuyenphatService.GetAllPhieuChuyenPhat(_workContext.NhaXeId, model.VanPhongGuiId, model.NgayTao, model.HangHoaInfo, model.TrangThai, model.PhieuVanChuyenId, model.VanPhongNhanId, model.TuNgay, model.DenNgay);
            //chi lay nhung phieu van chuyen tren xe
            items = items.Where(c => c.IsPhieuVanChuyenTrenXe == true).ToList();
            //loai bo nhung phieu neu co trong Phieu van chuyen
            if (model.OutPhieuVanChuyenId > 0)
            {
                var itemexcludes = _phieuchuyenphatService.GetAllPhieuChuyenPhat(_workContext.NhaXeId, 0, null, "", ENTrangThaiChuyenPhat.All, model.OutPhieuVanChuyenId).Select(c => c.Id).ToArray();
            ...
            foreach (var _id in arrId)
            {
                if (!string.IsNullOrEmpty(_id))
                {
                    ...
                    if (int.TryParse(_id, out id))
                    {
                        ...
                        var VanPhongNhanId = phieuchuyenphat.VanPhongNhanId;
                        ...
                        if (VanPhongNhanId == (int)ENVanPhongGuiTinNhan.VP_AnhHuyDatCang)
                            strsms = strsms + ". ĐT 19001876 ẤN PHÍM 1 LẠC LONG " +phieuchuyenphat.VanPhongNhan.DienThoaiGuiHang;
                        if (VanPhongNhanId == (int)ENVanPhongGuiTinNhan.VP_HaiDuong)
                            strsms = strsms + ". ĐT 19001876 ẤN PHÍM 2 HẢI DƯƠNG " + phieuchuyenphat.VanPhongNhan.DienThoaiGuiHang;
                        if (VanPhongNhanId == (int)ENVanPhongGuiTinNhan.VP_GiapBat)
                            strsms = strsms + ". ĐT 19001876 ẤN PHÍM 3 GIẢI PHÓNG " + phieuchuyenphat.VanPhongNhan.DienThoaiGuiHang;
                        if (VanPhongNhanId == (int)ENVanPhongGuiTinNhan.VP_AnhHuyDatCang)
                            strsms = strsms + ". ĐT 19001876 ẤN PHÍM 4 SÓ 91 KHUẤT DUY TIẾN "+ phieuchuyenphat.VanPhongNhan.DienThoaiGuiHang;
                        if (VanPhongNhanId == (int)ENVanPhongGuiTinNhan.VP_AnhHuyDatCang)
                            strsms = strsms + ". ĐT 19001876 ẤN PHÍM 5 số 2/293 KHUẤT DUY TIẾN " + phieuchuyenphat.VanPhongNhan.DienThoaiGuiHang;
                        ...
                        if (!string.IsNullOrEmpty(phieuchuyenphat.NguoiNhan.SoDienThoai))
                        {
                            ...
                            string sodienthoai = phieuchuyenphat.NguoiNhan.SoDienThoai;
                            var sub = sodienthoai.Substring(1, sodienthoai.Length-1);
                            sodienthoai = "84" + sub;
                            string _result = SendSMS(sodienthoai, strsms);
                            string _retmsg = "Lỗi";
        ...
        #endregion
        #region quan ly bang gia cuoc hang hoa
        public ActionResult QLBangGiaCuocHangHoa()
        {
            return View();
        }
        [NonAction]
        protected virtual void BangGiaCuocHangHoaToModel(BangGiaCuoc nvfrom, BangGiaCuocHangHoaModel nvto)
        {
            nvto.Id = nvfrom.Id;
        ...
        [HttpPost]
        public ActionResult BangGiaCuocHangHoaList(DataSourceRequest command)
        {
            var items = _phieuchuyenphatService.GetAllBangGiaCuocHangHoa(_workContext.NhaXeId);
            var gridModel = new DataSourceResult
            {
        ...
        public ActionResult BangCuocPhiThemMoi()
        {
            var model = new BangGiaCuocHangHoaModel();
            return View(model);
        }
        [HttpPost, ParameterBasedOnFormName("save-continue", "continueEditing")]
        public ActionResult BangCuocPhiThemMoi(BangGiaCuocHangHoaModel model)
        {
            var item = new BangGiaCuoc();
            item.NhaXeId = _workContext.NhaXeId;
            item.TenHangHoa = model.TenHangHoa;
            item.GiaCuoc = model.GiaCuoc;
            _phieuchuyenphatService.InsertBangGiaCuocHangHoa(item);
            return Content("OK");
        }
        #endregion
